extension Ion {
    @frozen @usableFromInline struct _FieldAccessor<ID, Failure>: ~Copyable
        where ID: CustomStringConvertible & Sendable, Failure: Error {
...
extension Ion._FieldAccessor {
    @inlinable func decode<T>(
        ...
        case .success(nil):
            throw Ion.FieldAccessError<ID>.undefined(self.id)
